...
const UserOptions: React.FC<UserOptionsProps> = ({ window, closeWindow }) => {
	return (
		<div className={`absolute right-3 ${window} top-12 z-50 bg-white w-4/5  pt-5 pb-5 rounded-3xl shadow-img flex flex-col justify-between`} >
			<button
				onClick={closeWindow}
			...
			<ul>
				...
				<li className='bg-white h-10 '>
					...
					>
						<div className='flex items-center justify-between'>
							<RiAdminLine className='inline text-2xl mr-3 text-blueSky'/> Admin
						</div>
						<IoIosArrowForward className=' text-blueSky' />
					</Link>
				</li>
